...
function RankProcessor() {
    ...
    const fetchRankRequests = async () => {
        ...
        try {
            ...
            if (data.length === 0) {
                setMessage({
                    text: 'No rank requests found in inbox',
                    isError: false
                });
            } else {
                setMessage({
                    text: `Found ${data.length} rank requests`,
                    isError: false
                });
            }
        } catch (err) {
    ...
    const sendCertificates = async () => {
        ...
        if (verifiedRequests.length === 0) {
            setMessage({
                text: 'No verified rank requests to process',
                isError: true
            });
            return;
        }
        ...
        try {
            const response = await fetch('http://localhost:8000/send-rank-certificates', {
                method: 'POST',
                headers: {
            ...
            console.error('Error sending rank certificates:', err);
            setMessage({
                text: `Error: ${err.message || 'Failed to connect to server'}`,
                isError: true
            });
    ...
    return (
        <div className="material-card">
            <h2 style={{ color: "#2E7D32" }}>Rank Certificate System</h2>
            <p>Process rank certificate requests from emails.</p>

            <div className="form-actions" style={{ marginBottom: '20px' }}>
                <button
                    className="button"
                    onClick={fetchRankRequests}
                    disabled={isLoading}
                    style={{
                ...
                >
                    {isLoading ? 'Fetching...' : 'Fetch Rank Requests'}
                </button>
            ...
            {requests.length > 0 && (
                <div className="request-table-container">
                    <h3 style={{ color: "#2E7D32" }}>Found Rank Requests</h3>
                    <table style={{
                        width: '100%',
...
export { RankProcessor };
export default RankProcessor;
